...
var int_compare = func(n1, n2 int) int {
	if n1 > n2 {
		return 1
	}
	if n1 < n2 {
...
func TestDiccionarioClaveDefault(t *testing.T) {
	...
	dicNum := TDADiccionario.CrearABB[int, string](int_compare)
...
func TestReemplazoDatoHopscotch(t *testing.T) {
	...
	dic := TDADiccionario.CrearABB[int, int](int_compare)
	for i := 0; i < 500; i++ {
		dic.Guardar(i, i)
	}
	for i := 0; i < 500; i++ {
...
func TestConClavesNumericas(t *testing.T) {
	...
	dic := TDADiccionario.CrearABB[int, string](int_compare)
	clave := 10
	valor := "Gatito"
